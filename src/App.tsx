...
type View = 'escola' | 'upload' | 'dados' | 'dashboard' | 'dashboard-geral' | 'calendario' | 'docentes-turmas' | 'cadastro-docentes' | 'areas-conhecimento' | 'cadastro-alunos' | 'cadastro-servidores' | 'relatorios' | 'horario';

export function App() {
  // Auth Hook
  const { user, loading: authLoading, login, logout, isAuthenticated } = useAuth();
  
  // Tema Hook (Modo Escuro)
    ...
    const carregarDadosIniciais = async () => {
      if (isAuthenticated && user && !dadosCarregados) {
        ...
        if (dados) {
          ...
          if (dados.atribuicoes && Array.isArray(dados.atribuicoes) && dados.atribuicoes.length > 0) {
            console.log('📚 Restaurando atribuições:', dados.atribuicoes.length);
            importData(dados.atribuicoes);
          }
          
          // Restaurar Escola - verificar se tem dados válidos
          if (dados.escola && typeof dados.escola === 'object' && Object.keys(dados.escola).length > 0) {
            console.log('🏫 Restaurando escola:', dados.escola);
            // Garantir que a escola tem o formato correto
            const escolaRestaurada = {
              nome: dados.escola.nome || '',
              cnpj: dados.escola.cnpj || '',
              codigoInep: dados.escola.codigoInep || '',
              email: dados.escola.email || '',
              telefone: dados.escola.telefone || '',
              endereco: dados.escola.endereco || {
                logradouro: '', numero: '', complemento: '',
                bairro: '', cidade: '', uf: '', cep: ''
              },
              tiposEnsino: dados.escola.tiposEnsino || [],
              anosSeries: dados.escola.anosSeries || [],
              turnos: dados.escola.turnos || [],
              aee: dados.escola.aee || {
                possui: false, salaRecursos: false, salaRecursosQuantidade: 0,
                deficienciasAtendidas: [], profissionaisAEE: 0, observacoes: ''
              },
              projetos: dados.escola.projetos || [],
              infraestrutura: dados.escola.infraestrutura || {
                acessibilidade: { rampa: false, elevador: false, banheiroAdaptado: false, pisoTatil: false, sinalizacaoBraille: false },
                espacos: { salaRecursos: false, salaRecursosQuantidade: 0, laboratorioInformatica: false, laboratorioCiencias: false, biblioteca: false, quadraEsportiva: false, quadraCoberta: false, auditorio: false, refeitorio: false, parqueInfantil: false },
                totalSalas: 0
              },
              diretor: dados.escola.diretor || '',
              viceDiretor: dados.escola.viceDiretor || ''
            };
            restaurarEscola(escolaRestaurada);
            // Também salvar no localStorage para garantir persistência
            localStorage.setItem('escola-cadastro', JSON.stringify(escolaRestaurada));
          }
          
          // Restaurar Docentes
          if (dados.docentes && Array.isArray(dados.docentes) && dados.docentes.length > 0) {
            console.log('👨‍🏫 Restaurando docentes:', dados.docentes.length);
            restaurarDocentes(dados.docentes);
          }
          
          // Restaurar Alunos
          if (dados.alunos && Array.isArray(dados.alunos) && dados.alunos.length > 0) {
            console.log('👨‍🎓 Restaurando alunos:', dados.alunos.length);
            restaurarAlunos(dados.alunos);
            // Também salvar no localStorage
            localStorage.setItem('gestao-escolar-alunos', JSON.stringify(dados.alunos));
          }
          
          // Restaurar Servidores
          if (dados.servidores && Array.isArray(dados.servidores) && dados.servidores.length > 0) {
            console.log('👷 Restaurando servidores:', dados.servidores.length);
            restaurarServidores(dados.servidores);
            // Também salvar no localStorage
            localStorage.setItem('gestao-escolar-servidores', JSON.stringify(dados.servidores));
          }
  ...
  // Removido useEffect de marcação de alterações
        ...
        `}>
          <div className="h-full flex flex-col">
            ...
            <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
              ...
              {/* Dashboard Geral */}
              <button
                onClick={() => { setCurrentView('dashboard-geral'); setSidebarOpen(false); }}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${
                  currentView === 'dashboard-geral'
              ...
              >
                ...
                <span className="flex-1 text-left font-medium">Dashboard Geral</span>
                <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700">NOVO</span>
              </button>

              {/* Calendário Escolar */}
              <button
                onClick={() => { setCurrentView('calendario'); setSidebarOpen(false); }}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${
              ...
              >
                ...
                <span className="flex-1 text-left font-medium">Calendário Escolar</span>
                <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700">NOVO</span>
              </button>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {currentView === 'dashboard-geral' && (
              <DashboardGeral
                ...
                escola={{
                  nome: escola?.nome || '',
                  tiposEnsino: escola?.tiposEnsino || [],
                  turnos: escola?.turnos?.map(t => ({ nome: t.tipo || '', ativo: t.ativo })) || [],
                  diretor: escola?.diretor || '',
                  endereco: escola?.endereco || { logradouro: '', numero: '', bairro: '', cidade: '', uf: '', cep: '', complemento: '' }
                }}
              />
            )}
