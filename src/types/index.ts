...
export type Parentesco = 'Mãe' | 'Pai' | 'Avô' | 'Avó' | 'Tio' | 'Tia' | 'Irmão' | 'Irmã' | 'Padrasto' | 'Madrasta' | 'Responsável Legal' | 'Outro';
...
export type TipoSanguineo = 'A+' | 'A-' | 'B+' | 'B-' | 'AB+' | 'AB-' | 'O+' | 'O-' | 'Não informado';
...
export interface FichaSaude {
  ...
  autorizaAtendimentoEmergencia: boolean;
  autorizaMedicacao: boolean;
}

// Ocorrência/Atendimento do Aluno
export type TipoOcorrencia = 'Disciplinar' | 'Pedagógico' | 'Saúde' | 'Família' | 'Bullying' | 'Evasão' | 'Elogio' | 'Reunião' | 'Atendimento AEE' | 'Outro';
...
export type NivelAcesso = 'admin' | 'diretor' | 'coordenador' | 'secretario' | 'professor' | 'funcionario' | 'visualizador';

export interface PermissoesUsuario {
  // Cadastros
  cadastroEscola: boolean;
  cadastroDocentes: boolean;
  cadastroAlunos: boolean;
  cadastroServidores: boolean;
  // Atribuições e Horários
  atribuicaoAulas: boolean;
  geradorHorarios: boolean;
  // Visualização
  dashboardGeral: boolean;
  calendario: boolean;
  relatorios: boolean;
  // Backup
  backup: boolean;
